...
RFC-009 adds on-demand voice input/output with these extra boundaries:

- Speaker verification runs before transcription, not after: a voice that does not match the enrolled owner's stored embedding is rejected and no transcript is ever produced for it, not merely discarded afterward. `voice.*` has no AI-callable tool surface of its own for this — a transcript is handed to the exact same `app.brain.router.route_command` path a typed message uses, so it carries no more authority than typed input and goes through the identical Risk Analyzer/approval flow. Speaking a request never auto-executes anything a typed request wouldn't.
- Enrollment is closed-set and opt-in, mirroring RFC-007E's face-enrollment shape: one local speaker embedding for the owner only (`data/voice_profile.json`), never raw audio, deleted only through `voice forget me` (routed through the same confirmation flow as destructive power actions, since it deletes biometric data).
- `/voice/turn` (the phone push-to-talk route) shares RFC-010's Tailscale-bound listener and its `location_bind_host`/`location_shared_secret` — never `0.0.0.0`, never reachable without the correct bearer token, exactly like `/location/overland`.
- Anti-replay/liveness detection is explicitly out of scope for this version (documented, not silently assumed solved): a played recording of the owner's voice may currently pass verification. Voice alone is never treated as sufficient to authorize a MEDIUM/HIGH-risk action — that already requires `approve plan` regardless of who or what asked.
- No voice other than the enrolled owner's is ever captured, transcribed, or analyzed, even transiently — this is enforced by verifying before transcription, not merely a policy statement.
