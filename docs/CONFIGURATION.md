...
- `voice_verification_threshold`
  Minimum cosine similarity (0.0-1.0) between a live utterance's speaker embedding and the enrolled owner's stored embedding before a voice turn is accepted. Default `0.6`, recalibrated from an original default of `0.75` after real-hardware diagnostics showed that value rejecting the genuine enrolled owner: enrollment's own internal pairwise sample similarity was 0.50-0.67, and a live genuine-owner verification attempt scored 0.7311 -- just under the old cutoff. `speechbrain`'s own `SpeakerRecognition.verify_batch()` reference implementation for this exact `spkrec-ecapa-voxceleb` checkpoint defaults to `threshold=0.25`; raw (non length/score-normalized) ECAPA cosine similarities are evidently calibrated by the model's own authors well below `0.75`. `0.6` sits below every genuine-match score observed so far while staying well above that `0.25` reference boundary. This is a data-driven starting point, not a universal constant -- a different microphone, room, or owner may warrant retuning it via `config set voice_verification_threshold <value>`, guided by the similarity scores `voice enroll`/`voice talk` now log to `logs/jarvis.log`.
- `voice_input_device`
